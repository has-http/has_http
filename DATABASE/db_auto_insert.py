...
teach_insert_query = 'INSERT INTO teach VALUES (%s, %s, %s, %s, %s, %s, %s);'

course_rows = []
teach_rows = []

for i, line in enumerate(rdr):
    c_name,  c_classify, b_code_list,  c_count = line
    c_no = str(3001 + i)
    course_rows.append((c_no, c_name, c_classify,  c_count))
    for j, s in enumerate(b_code_list.split()):
        teach_rows.append((str(c_count) + s + '3', c_no, c_name, str(j+1), '0', '0', '30'))
